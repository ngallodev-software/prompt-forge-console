...
| `updateRule` | `updateRule(id: string, patch: Partial<Pick<Rule, "enabled" \| "priority">>): Promise<{ ok: boolean; id: string } & Partial<Pick<Rule, "enabled" \| "priority">>>` | `PATCH /console/rules/{id}` | On backend error, `STRICT_BACKEND` rethrows; otherwise it waits briefly and echoes the patch back with `{ ok: true, id, ...patch }`. |
